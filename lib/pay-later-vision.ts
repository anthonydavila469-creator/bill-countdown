/**
 * Pay Later vision scanner — Claude Sonnet 4.6 (vision) with tool-use forcing.
 *
 ...
 *
 * Image (vision) fallback model: `claude-sonnet-4-6` — the newest Sonnet,
 * live-confirmed served by our Anthropic account. The on-device-OCR fast
 * path extracts from text with Haiku (`PAY_LATER_TEXT_MODEL`).
 */
...
// Image (vision) fallback model. Claude Sonnet 4.6 — the newest Sonnet,
// confirmed served by our Anthropic account via a live probe (2026-05-31:
// claude-sonnet-4-6 returned 200 and resolved to itself). @anthropic-ai/sdk
// 0.71.2 doesn't list `claude-sonnet-4-6` in its Model union, but the union
// ends in `(string & {})` so the SDK passes the ID through unchanged.
// NOTE: this intentionally diverges from the app-wide claude-sonnet-4-20250514
// pin — only the Pay Later vision fallback runs 4.6. This path is rare (most
// scans take the Haiku on-device-OCR fast path), so the divergence is scoped.
