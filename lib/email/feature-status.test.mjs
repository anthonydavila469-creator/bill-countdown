...
const FORBIDDEN_IMPORTS = [
  ...
  '@/lib/bill-extraction',
  '@/lib/parser/',
  // Legacy Claude email-extraction pipeline (P1-2) — a disabled route reaching
  // this means user-submitted email bodies can still hit the AI extractor.
  '@/lib/ai/process-bills',
  '@/lib/ai/extract-bill',
];
...
const DISABLED_HELPER_ROUTES = [
  ...
  // Mailbox-scan entry points that called fetchProviderEmails() directly.
  'app/api/extraction/scan-inbox/route.ts',
  'app/api/suggestions/route.ts',
  // Legacy AI email-parse route (P1-2): accepted user-submitted email bodies
  // and fed them into the Claude email-extraction pipeline.
  'app/api/ai/parse-bills/route.ts',
];
...
test('legacy AI email-extraction logs are DEBUG-gated, never raw in production (P1-2)', () => {
  const src = read('lib/ai/extract-bill.ts');
  // The non-production gate must exist and key off NODE_ENV.
  assert.match(
    src,
    /const DEBUG_EXTRACTION_LOGS = process\.env\.NODE_ENV !== 'production'/,
    'extract-bill.ts must define a non-production debug-log gate',
  );
  // Every console.* that touches email-derived data must sit behind the gate.
  // For each sensitive marker, walk up to its immediately-enclosing block
  // opener (first less-indented line ending in `{`) and require it to be the
  // DEBUG_EXTRACTION_LOGS guard. Indentation-based so template-literal braces
  // like ${email.subject} don't confuse it.
  const lines = src.split('\n');
  const indentOf = (line) => line.length - line.trimStart().length;
  for (const marker of [
    '[EXTRACT] Email',
    '[EXTRACT] Result',
    '[extractBillFromEmail] Extracted',
    '[extractBillFromEmail] Skipped',
    'Failed to parse response for',
  ]) {
    const lineIdx = lines.findIndex((l) => l.includes(marker));
    assert.notEqual(lineIdx, -1, `expected log marker "${marker}" to still exist`);
    const markerIndent = indentOf(lines[lineIdx]);
    let enclosing = null;
    for (let j = lineIdx - 1; j >= 0; j--) {
      const line = lines[j];
      if (line.trim() === '') continue;
      if (indentOf(line) < markerIndent && line.trimEnd().endsWith('{')) {
        enclosing = line;
        break;
      }
    }
    assert.ok(enclosing, `could not find enclosing block for "${marker}"`);
    assert.match(
      enclosing,
      /if \(DEBUG_EXTRACTION_LOGS\)/,
      `log "${marker}" must be inside an if (DEBUG_EXTRACTION_LOGS) block, found: ${enclosing.trim()}`,
    );
  }
});
