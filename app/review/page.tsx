...
export default function ReviewQueuePage() {
  ...
  const fetchReviewQueue = useCallback(async () => {
    ...
    try {
      const response = await apiFetch('/api/extraction/review-queue');
        ...
        <div className="p-6 border-b border-white/5">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-white tracking-tight">
              Bill<span className="text-violet-400">Countdown</span>
            </span>
          </Link>
        </div>
          ...
          <ul className="space-y-1">
            ...
            <li>
              ...
              >
                ...
                Review Queue
                {items.length > 0 && (
                  <span className="ml-auto px-2 py-0.5 rounded-full bg-violet-500/20 text-violet-400 text-xs font-medium">
                    {items.length}
                  </span>
                )}
              </Link>
        ...
        <header className="sticky top-0 z-40 bg-[#0F0A1E]/80 backdrop-blur-xl border-b border-white/5">
          <div className="flex items-center justify-between px-6 h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              {items.length > 0 && (
                <span className="px-2.5 py-1 rounded-full bg-violet-500/20 text-violet-400 text-sm font-medium">
        ...
        <div className="p-6">
          ...
          {isRefreshing && items.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 px-4">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-violet-500/20 to-amber-500/20 flex items-center justify-center mb-6 animate-pulse">
